...
````{important}
The MD20 is equipped with a shunt resistor different than MD80, suited for precise current sensing in smaller range. As a result all the configuration files (.cfg) dedicated for MD20 **MUST** contain `[hardware]` section with `shunt resistance` parameter equal to 0.004, ie.:
```
[hardware]
...
MD20 is equipped with seven mounting holes. Please refer to the technical drawing above to find
...
For power supply and FDCAN bus connection two _2200980471 Molex Micro-lock_ connectors are
...
encoders. For external encoder connection via the SPI _530480650 Molex PicoBlade_ connectors is
provided.
...
```{note}
For all MD20 hardware revisions we recommend using [MABs thermistors](https://www.mabrobotics.pl/product-page/thermistor). Using other thermistors may result in imprecise temperature readout.
```
...
efficient power converters and MOSFETs with best-in-class figure of merit.
...
Temperature vs Time characteristic (note that this characteristic was split into two current
ranges):
