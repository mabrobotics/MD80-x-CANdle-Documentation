...
* - Torque Bandwidth (adjustable)
  - 50 Hz - 2.5 kHz
````
## Difference between input current and phase current
The MD80 comes with cable set to connect it to power supply and CAN network, with usage of CANdle or to custom hardware. To avoid confusion during wiring it is important to understand the difference between motor phase current and input current from the power source.
Input current depends strongly on the total mechanical power delivered by the actuator, so increasing the motor torque and velocity increases that quantity. 
In the case of motor phase current, it is produced by the motor controller, and the best way to imagine the relation is to think about the motor controller as a kind of three-phase rotating DC/DC converter - it sets the output current to the desired level. The voltage is set by the PWM on the proper values by the current regulator.
Below there is summary that presents the basic idea behind it:
```{figure} ./images/MD80/motor_vs_input_current.jpg
